...
For NestJS specifically: The entry point is a @Controller() that handles the HTTP routing and validation, which strictly delegates the business logic to a co-located @Injectable() handler class. Both must be registered in the domain's @Module().
